    ...
    public class CustomerRepository
    {
        ...
        public void UpdateCustomer(Customer customer)
        {
            using (var conn = new SqlConnection(_connectionString))
            {
                conn.Open();
                var cmd = new SqlCommand(
                    "UPDATE Customers SET FirstName=@FirstName, LastName=@LastName, Email=@Email, Phone=@Phone, Address=@Address WHERE CustomerId=@CustomerId", conn);
                cmd.Parameters.AddWithValue("@FirstName", customer.FirstName);
                cmd.Parameters.AddWithValue("@LastName", customer.LastName);
                cmd.Parameters.AddWithValue("@Email", customer.Email);
                cmd.Parameters.AddWithValue("@Phone", customer.Phone ?? (object)DBNull.Value);
        ...
        public void DeleteCustomer(int customerId)
        {
            ...
            {
                conn.Open();
                var cmd = new SqlCommand("DELETE FROM Customers WHERE CustomerId=@CustomerId", conn);
        ...
        public void AddUser(string username, string password, string fullName, string email)
        {
            ...
            using (var conn = new SqlConnection(_connectionString))
            {
                conn.Open();
                var cmd = new SqlCommand(
                    "INSERT INTO Users (Username, PasswordHash, FullName, Email) VALUES (@Username, @PasswordHash, @FullName, @Email)", conn);
